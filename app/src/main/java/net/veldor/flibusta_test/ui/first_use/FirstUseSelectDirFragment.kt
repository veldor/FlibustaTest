...
import net.veldor.flibusta_test.databinding.FragmentFirstUseSelectDirBinding
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                if (data != null && data.extras != null && data.extras!!.containsKey("data")) {
                    ...
                    if (file.isDirectory && PreferencesHandler.instance.setDownloadDir(
                            folderLocation
                        )
                    ) {
                        Toast.makeText(
        ...
        binding.useDefaultDirBtn.setOnClickListener {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                if (file.isDirectory && file.canWrite()) {
                    PreferencesHandler.instance.setDownloadDir(
                        file.absolutePath
                    )
                    goToNext()
                } else {
