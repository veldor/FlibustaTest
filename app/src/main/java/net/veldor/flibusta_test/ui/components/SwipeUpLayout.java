package net.veldor.flibusta_test.ui.components;
 ...
 * </p>
 * <p>
 * * SwipeRefreshLayout следует использовать всякий раз, когда пользователь может обновить
 * * содержимое представления с помощью вертикального жеста салфетки. Деятельность, которая
 * * экземпляр этого представления должен добавить OnRefreshListener для уведомления
 * * всякий раз, когда жест пролистывания для обновления завершен. SwipeRefreshLayout
 * * будет уведомлять слушателя каждый раз, когда жест снова завершается;
 * * слушатель несет ответственность за правильное определение того, когда на самом деле
 * * инициировать обновление его содержимого. Если слушатель решит, что следует
 * * не быть обновлением, он должен вызывать setRefreshing(false) для отмены любого визуального
 * * указание на обновление. Если активность хочет показать только прогресс
 * * анимация, она должна вызывать setRefreshing(true). Чтобы отключить жест и
 * * анимация прогресса, вызов setEnabled(false) в представлении.
 * * <р>
 * * Этот макет следует сделать родительским для представления, которое будет обновляться как
 * * результат жеста и может поддерживать только одного прямого потомка. Этот вид будет
 * * также станет целью жеста и будет вынужден соответствовать обоим
 * * Ширина и высота указаны в этом макете. SwipeRefreshLayout не
 * * обеспечить доступность событий; вместо этого должен быть предоставлен пункт меню, позволяющий
 * * обновление контента везде, где используется этот жест.
 * * </p>
 */
public class SwipeUpLayout extends ViewGroup implements NestedScrollingParent3,
    ...
    private static final String LOG_TAG = SwipeUpLayout.class.getSimpleName();
    ...
    // Расстояние в пикселях, на которое может уйти касание, прежде чем мы подумаем, что пользователь прокручивает
    ...
    // Если включена вложенная прокрутка, общая сумма, которую необходимо
    // потребляется этим, так как вложенный родительский элемент прокрутки используется вместо
    // прокрутка определяется событиями MOVE в обработчике onTouch
    ...
    // насколько я понимаю, это- текущий сдвиг прогрессбара от начального значения
    int mCurrentTargetOffsetTop;

    // тут палец находится в начале движения
    private float mInitialMotionY;

    // тут стоит палец в начале жеста
    private float mInitialDownY;
    // указатель процесса свайпа?
    ...
    // изначальное смещение прогрессбара
    ...
    private boolean mReadyToRefreshNotified = false;

    // возвращение в стартовую позицию
    void reset() {
        mReadyToRefreshNotified = false;
    ...
    private void setColorViewAlpha() {
        mCircleView.getBackground().setAlpha(SwipeUpLayout.MAX_ALPHA);
        mProgress.setAlpha(SwipeUpLayout.MAX_ALPHA);
    }
     ...
     */
    public SwipeUpLayout(@NonNull Context context) {
        this(context, null);
    }
     ...
     */
    public SwipeUpLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        // тут конструктор
        // Расстояние в пикселях, на которое может уйти касание, прежде чем мы подумаем, что пользователь прокручивает
        ...
        // f you need the onDraw method to be called inside a ViewGroup, you just set the flag to false like so: setWillNotDraw(false).
        setWillNotDraw(false);

        // интерполятор замедления?
     ...
     * <p>
     * Сообщите виджету, что состояние обновления изменилось. Не вызывайте это, когда
     * обновление запускается жестом смахивания.
     *
    ...
    private void ensureTarget() {
        ...
        // out yet.
        // Не утруждайте себя получением высоты родителя, если родитель еще не размещен.
        if (mTarget == null) {
    ...
    /**
     * Set a callback to override  method. Non-null
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        // перехвачено движение
        // эта функция просто находит целевой элемент
        ensureTarget();
        ...
        if (mReturningToStart && action == MotionEvent.ACTION_UP) {
            // тут дейтствие с возвращением на стартовую позицию
            Log.d("surprise", "onInterceptTouchEvent 799:  return to start detected");
            mReturningToStart = false;
        }
        if (!isEnabled() || mReturningToStart || canChildScrollDown()
                || mRefreshing || mNestedScrollInProgress) {
            // Fail fast if we're not in a state where a swipe is possible
            // Откат, если мы не находимся в состоянии, когда возможен свайп
            return false;
        }

        switch (action) {
            case MotionEvent.ACTION_DOWN:
                int offset = mOriginalOffsetTop - mCircleView.getTop();
                // тут считается смещение прогрессбара на начало нажатия
                setTargetOffsetTopAndBottom(offset);
                // получу уникальный идентификатор жеста
                mActivePointerId = ev.getPointerId(0);

                // тут скажу, что процесс свайпа неактивен
                mIsBeingDragged = false;

                pointerIndex = ev.findPointerIndex(mActivePointerId);
                // тут пока непонятно, проверяется порядковый номер указателя (?) и если он больше 0- действие обрывается
                if (pointerIndex > 0) {
                    return false;
                }
                // ага, это- точка, в которой стоит палец на момент начала жеста
                mInitialDownY = ev.getY(pointerIndex);
                break;

            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId == INVALID_POINTER) {
                    // тут видимо защита от неверного жеста- когда есть движение, но непонятно, с чем его связать
                    Log.e(LOG_TAG, "Got ACTION_MOVE event but don't have an active pointer id.");
                    return false;
                }

                pointerIndex = ev.findPointerIndex(mActivePointerId);
                // тут ещё одна проверка, на то, является ли жест тем, что мы обрабатываем
                if (pointerIndex < 0) {
                    return false;
                }
                // тут определено, где сейчас находится палец
                final float y = ev.getY(pointerIndex);
                // фиксирую начало движения
                startDragging(y);
                break;

            case MotionEvent.ACTION_POINTER_UP:
                Log.d("surprise", "onInterceptTouchEvent 857:  не понял какое действие");
                onSecondaryPointerUp(ev);
                break;

            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
                Log.d("surprise", "onInterceptTouchEvent 846:  отпустили палец");
                // отмечаем, что движение закончено
                mIsBeingDragged = false;
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        if (Math.abs(overscrollTop) < mTotalDragDistance) {
            if(mTotalDragDistance - Math.abs(overscrollTop) < 30){
                mReadyToRefreshNotified = false;
            }
            ...
            if (!mReadyToRefreshNotified) {
                mReadyToRefreshNotified = true;
                mCircleView.performHapticFeedback(
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        // тут, видимо, проверка на реверсивное движение для возвращения прогрессбара, пока пропустим
        if (mReturningToStart && action == MotionEvent.ACTION_DOWN) {
            Log.d("surprise", "onTouchEvent 1286:  returning to start detected");
            mReturningToStart = false;
        }

        if (!isEnabled() || mReturningToStart || canChildScrollDown()
                || mRefreshing || mNestedScrollInProgress) {
            // тут- проверка на отменённое действие
            Log.d("surprise", "onTouchEvent 1292:  action not available!");
            // Fail fast if we're not in a state where a swipe is possible
            return false;
        }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                // вот теперь всё понятно. Тут- нажатие.
                Log.d("surprise", "onTouchEvent 1304:  thumb is up");
                // указываем ID жеста, пока не понял
                mActivePointerId = ev.getPointerId(0);
                Log.d("surprise", "onTouchEvent 1308:  active pointer id is " + mActivePointerId);
                // отметили, что движение неактивно
                mIsBeingDragged = false;
                break;

            case MotionEvent.ACTION_MOVE: {
                // тут всё понятно. Отсекаю левые движения
                pointerIndex = ev.findPointerIndex(mActivePointerId);
                if (pointerIndex < 0) {
                    Log.d("surprise", "onTouchEvent 1317:  move with wrong pointer");
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overscrollTop > 0) {
                        ...
                        // Пока спиннер тащится вниз, наш родитель не должен пытаться
                        // перехватывать события касания. Это резко остановит жест перетаскивания.
            ...
            case MotionEvent.ACTION_POINTER_UP:
                Log.d("surprise", "onTouchEvent 1349:  pointer down");
                onSecondaryPointerUp(ev);
                break;

            case MotionEvent.ACTION_UP: {
                Log.d("surprise", "onTouchEvent: отпустили палец");
                pointerIndex = ev.findPointerIndex(mActivePointerId);
                Log.d("surprise", "onTouchEvent 1356:  Идентификатор нажатия: " + pointerIndex);
                if (pointerIndex < 0) {
                    Log.e(LOG_TAG, "Got ACTION_UP event but don't have an active pointer id.");
                    return false;
                }
                if (mIsBeingDragged) {
                    // посчитаю точку, где сейчас находится палец
                    final float y = ev.getY(pointerIndex);
                    Log.d("surprise", "onTouchEvent 1364:  current finger position is " + y);
                    // todo разобраться тут после того, как разберусь с движением
    ...
    private void startDragging(float y) {
        // тут самое интересное вроде бы
        // инвертирую событие для поддержания логики
        final float yDiff = mInitialDownY - y;
        if (yDiff > mTouchSlop && !mIsBeingDragged) {
            Log.d("surprise", "startDragging 1368:  register start dragging");
            // тут проверю, что действие больше погрешности и начало движения ещё не зарегистрировано
            //  зарегистрирую точку начала движения
            mInitialMotionY = mInitialDownY + mTouchSlop;
            // то есть это выполняется всего один раз, зарегистрирую начало движения
            mIsBeingDragged = true;
            // тут какая-то фишка с прозрачностью :)
    ...
    void setTargetOffsetTopAndBottom(int offset) {
        // тут вывожу прогрессбар на передний план, сдвигаю его на заданное количество пикселей сверху и сохраняю его позицию в переменную
    ...
    public interface OnChildScrollUpCallback {
         ...
         */
        boolean canChildScrollDown(@NonNull SwipeUpLayout parent, @Nullable View child);
    }
}
