...
class DownloadScheduleStatementFragment : Fragment() {
    ...
    private fun unbindObservers() {
        DownloadHandler.instance.downloadInProgress.removeObservers(viewLifecycleOwner)
        DatabaseInstance.instance.mDatabase.booksDownloadScheduleDao().allBooksLive?.removeObservers(
            viewLifecycleOwner
        )
        DownloadHandler.instance.liveBookDownloadProgress.removeObservers(viewLifecycleOwner)
    }
}