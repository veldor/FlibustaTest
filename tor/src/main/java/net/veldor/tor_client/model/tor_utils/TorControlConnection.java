...
import net.veldor.tor_client.model.exceptions.TorControlError;
import net.veldor.tor_client.model.exceptions.TorControlSyntaxError;
...
public class TorControlConnection implements TorControlCommands {
    ...
    protected void handleEvent(ArrayList<ReplyLine> events) {
        ...
        for (Iterator<ReplyLine> i = events.iterator(); i.hasNext(); ) {
            ...
            if (tp.equals("CIRC")) {
                List<String> lst = Bytes.INSTANCE.splitStr(null, rest);
    ...
    */
    ...
    public String getLastBootstrapLog()
    {
        return ((OnionProxyManagerEventHandler)handler).getLastBootstrapLog();
    }
    ...
    public void authenticate(byte[] auth) throws IOException {
        String cmd = "AUTHENTICATE " + Bytes.INSTANCE.hex(auth) + "\r\n";
