...
abstract public class OnionProxyContext {
    ...
    protected final static String OBFS4_NAME = "libobfs4proxy.so";
    ...
    public OnionProxyContext(File workingDirectory) {
        ...
        obfsFile = new File(getWorkingDirectory(), OBFS4_NAME);
        // add a custom bridge to file
    ...
    public void installFiles() throws IOException, InterruptedException {
        ...
        FileUtilities.cleanInstallOneFile(getAssetOrResourceByName(OBFS4_NAME), obfsFile);
        Log.d("surprise", "installFiles: file installed");
        if (obfsFile.exists()) {
                ...
                if (bridgesFile.exists() && bridgesFile.isFile() && bridgesFile.length() > 0) {
                    ...
                    try (BufferedReader br = new BufferedReader(new FileReader(bridgesFile))) {
                        for (String line; (line = br.readLine()) != null; ) {
                            if (!line.isEmpty()) {
                                fileOutputStream.write("Bridge ".getBytes());
